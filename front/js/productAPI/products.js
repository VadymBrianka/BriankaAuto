import { attachEventHandlerClass, attachEventHandler, backURL, getPage, getUser, setPageCount } from "../config.js";
import { confirmModal, waitForRemoveButtonPress } from "../modals/confirmationModal.js";
import { dropDownClose } from "../modals/main.js";
import { convertModalToShow, convertModalToEdit, openProductModalWithCreate, openProductModalWithShow, productModal, renderProductCategoriesOptions } from "../modals/productModal.js";
...
export const  btnRender = () => {
    ...
    if ( user && user.isAdmin) {
        btnContainer.innerHTML = `<button type="button" class="nav-link" id="createBtn">Create New Car</button>`;
    }
    ...
    .then( data => {                                         // Парсимо [object Promise]
            ...
            if (data.items.length) {
                data.items.forEach(product => {
                    ...
                    attachEventHandlerClass('showButton', 'click', openProductModalWithShow);
                });
            }
            else {
...
export const editProduct = (product) => {
    ...
    document.getElementById('productMileage').value = product.mileage;
    document.getElementById('productEngineCapacity').value = product.engineCapacity;
...
//
// ***** Показ даних продукту
//
export const showInfoAboutProduct = (product) => {
    convertModalToShow();
    // Шукаємо, яка прийшла категорія, і вибираємо її
    const categoryListBox = document.querySelector('#producCategory');
    const options = Array.from(categoryListBox.options);
